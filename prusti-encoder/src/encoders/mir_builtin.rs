    ...
    fn handle_un_op<'vir>(
        ...
        match op {
            mir::UnOp::Neg | mir::UnOp::Not => {
                ...
                let name = vir::vir_format_identifier!(vcx, "mir_unop_{op:?}_{}", int_name(operand_ty));
            ...
            mir::UnOp::PtrMetadata => {
                ...
                let body = match operand_ty.peel_refs().kind() {
                    ty::TyKind::Slice(..)
                    | ty::TyKind::Array(..) => {
                        let ty_task = RustTyDecomposition::from_ty(operand_ty.peel_refs(), vcx.tcx(), GParams::empty());
                        let operand_array_pure = deps.require_dep::<TyUsePureEnc>(ty_task)?.expect_array();
                        let snap_arg = vcx.mk_local_ex(snap_arg_decl);
                        ...
                        Some((prim_res_ty.prim_to_snap)(operand_array_pure.len(operand_value).upcast_ty()))
                    }
                    _ => None,
                };
