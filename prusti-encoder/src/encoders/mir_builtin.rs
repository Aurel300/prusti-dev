...
use vir::{CSnap, CallableIdn, CastType, FunctionIdn};
...
impl MirBuiltinEnc {
    ...
    fn handle_un_op<'vir>(
        ...
        if !e_ty.is_float() {
            ...
            let fl_res_ty = e_ty.expect_float();
            match op {
                mir::UnOp::Not => unreachable!(),
                mir::UnOp::Neg => {
                    let res = (fl_res_ty.fp_neg)(snap_arg);
    ...
    fn handle_bin_op<'vir>(
        ...
        if !e_l_ty.is_float() {
            ...
            let val: &'vir vir::ExprGenData<'vir, (), !, CSnap> = (prim_res_ty.prim_to_snap)(val);
            Ok(vcx.mk_function(
                function,
                (lhs_decl, rhs_decl),
            ...
            let fl_l_ty = e_l_ty.expect_float();
            let e_l_ty_snap = e_l_ty.snapshot.downcast_ty();
            let e_r_ty_snap = e_r_ty.snapshot.downcast_ty();
            let e_res_ty_snap = e_res_ty.snapshot.downcast_ty();
            let name = vir::vir_format_identifier!(
            ...
            match op {
                Add => {
                    let res =
                        (fl_l_ty.fp_add)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                ...
                Sub => {
                    let res =
                        (fl_l_ty.fp_sub)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(res)))
                }
                ...
                Mul => {
                    let res =
                        (fl_l_ty.fp_mul)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(res)))
                }
                ...
                Div => {
                    let res =
                        (fl_l_ty.fp_div)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(res)))
                }
                Rem => {
                    ...
                    let div_res = (fl_l_ty.fp_div)(float_lhs, float_rhs);
                    let div_trunc = (fl_l_ty.fp_trunc)(div_res);
                    let mul_res = (fl_l_ty.fp_mul)(div_trunc, float_rhs);
                    let res = (fl_l_ty.fp_sub)(float_lhs, mul_res);
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(res)))
                }
                BitXor => unreachable!(),
                BitAnd => unreachable!(),
                ...
                Eq => {
                    let prim_res =
                        (fl_l_ty.fp_eq)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(prim_res.upcast_ty());
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
                Lt => {
                    let prim_res =
                        (fl_l_ty.fp_lt)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(prim_res.upcast_ty());
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
                Le => {
                    let prim_res =
                        (fl_l_ty.fp_leq)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(prim_res.upcast_ty());
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
                Ne => {
                    ...
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(neq);
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
                Ge => {
                    let prim_res =
                        (fl_l_ty.fp_geq)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(prim_res.upcast_ty());
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
                Gt => {
                    let prim_res =
                        (fl_l_ty.fp_gt)(vcx.mk_local_ex(lhs_decl), vcx.mk_local_ex(rhs_decl));
                    let val = (e_res_ty.expect_primitive().prim_to_snap)(prim_res.upcast_ty());
                    Ok(vcx.mk_function(function, (lhs_decl, rhs_decl), &[], &[], None, Some(val)))
                }
