...
impl TaskEncoder for MirPureEncoder {
    ...
    where
        F: FnOnce(&'vir task_encoder::CacheRef<'tcx, 'vir, MirPureEncoder>) -> R,
    {
        CACHE.with(|cache| {
...
impl<'tcx, 'vir: 'enc, 'enc> Encoder<'tcx, 'vir, 'enc> {
    ...
    fn encode_body(&mut self) -> ExprRet<'vir> {
        ...
        let (_, update) = self.encode_cfg(&init.versions, mir::START_BLOCK, None);
    ...
    fn encode_cfg(
        ...
        debug_assert!(
            match (dominators.immediate_dominator(curr), branch_point) {
        ...
        match &term.kind {
            ...
            mir::TerminatorKind::SwitchInt { discr, targets } => {
                ...
                let phi_expr = targets.iter().zip(updates.into_iter()).fold(
                    ...
                    |expr, ((cond_val, target), (_, branch_update))| {
                        self.vcx
            ...
            mir::TerminatorKind::Call {
                ...
                let expr = match func_ty.kind() {
                    &TyKind::FnDef(def_id, arg_tys) => {
                        ...
                        if is_pure {
                            let pure_func = self
                                .deps
                                .require_ref::<crate::encoders::MirFunctionEncoder>((
                                    def_id,
                                    arg_tys,
                                    self.def_id,
                                ))
                ...
                // walk rest of CFG
                let (end, end_update) =
                    self.encode_cfg(&new_curr_ver, target.unwrap(), branch_point);
        ...
        match rvalue {
            ...
            mir::Rvalue::BinaryOp(op, box (l, r)) => {
                ...
                ty_rvalue.apply(
                    ...
                    [self
                        .vcx
                        .alloc(ExprRetData::BinOp(self.vcx.alloc(vir::BinOpGenData {
                            kind: op.into(),
            ...
            mir::Rvalue::UnaryOp(op, expr) => {
                ...
                ty_rvalue.apply(
                    ...
                    [self
                        .vcx
                        .alloc(ExprRetData::UnOp(self.vcx.alloc(vir::UnOpGenData {
                            kind: op.into(),
    ...
    fn encode_prusti_builtin(
        ...
        match builtin.expect("call to unknown non-pure function in pure code") {
            PrustiBuiltin::SnapshotEquality => {
                ...
                let eq_expr =
                    self.vcx
                        .alloc(vir::ExprGenData::BinOp(self.vcx.alloc(vir::BinOpGenData {
                            kind: vir::BinOpKind::CmpEq,
                            lhs,
                            rhs,
                        })));
            ...
            PrustiBuiltin::Forall => {
                ...
                let forall = bool.prim_to_snap.apply(
                    self.vcx,
                    [self
                        .vcx
                        .alloc(ExprRetData::Forall(self.vcx.alloc(vir::ForallGenData {
                            qvars,
                            triggers: &[], // TODO
