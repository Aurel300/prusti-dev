use crate::encoders::{
    TyUseImpureEnc,
    ty::{
        RustTyDatas, RustTyDecomposition, StructType,
...
pub(crate) fn ty_impure_variant<'vir>(
    ...
    if data.struct_type == StructType::Unique {
        ...
        let rawptr_ref = nonnull.fields[0].field_ref((field_accessors[0].ref_to_field_ref)(
            ref_self,
            builder.params.ty_exprs(),
            builder.params.const_exprs(),
        ));
        let nonnull_ty = data.fields[0]
            .0
            .decompose(task_key.0.params)
            .ty
            .expect_structlike();
        ...
        let unfolding_expr = builder.vcx.mk_unfolding_expr(
            fields[0].ref_to_pred_app(
                (field_accessors[0].ref_to_field_ref)(
                    ref_self,
                    builder.params.ty_exprs(),
                    builder.params.const_exprs(),
                ),
    ...
    let pred_expr = builder.vcx.mk_conj(&pred_vec);
