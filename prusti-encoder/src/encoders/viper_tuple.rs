...
impl TaskEncoder for ViperTupleEncoder {
    ...
    where
        F: FnOnce(&'vir task_encoder::CacheRef<'tcx, 'vir, ViperTupleEncoder>) -> R,
    {
        CACHE.with(|cache| {
    ...
    > {
        ...
        vir::with_vcx(|vcx| {
            ...
            let elem_getters = elem_names
                .iter()
                .map(|name| FunctionIdent::new(name, UnaryArity::new([domain_type])))
                .collect::<Vec<_>>();
            ...
            if !is_unit_tuple {
                ...
                let qvars_ex = (0..*task_key)
                    ...
                    .collect::<Vec<_>>();
                let cons_call = constructor.apply(
                    vcx,
                    &qvars_names
                        .iter()
