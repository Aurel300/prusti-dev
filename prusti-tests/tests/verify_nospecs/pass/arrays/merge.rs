...
fn merge(a: [i32; 3], b: [i32; 3]) -> [i32; 6] {
    ...
    while res_pos < res.len() {
        // indices
        /*body_invariant*/drop(0 <= a_pos && a_pos <= 3);
        /*body_invariant*/drop(0 <= b_pos && b_pos <= 3);
        /*body_invariant*/drop(0 <= res_pos && res_pos < 6);
        /*body_invariant*/drop(a_pos + b_pos == res_pos);

        // subsequences and partial result sorted
        /*body_invariant*/drop(sorted3(&a));
        /*body_invariant*/drop(sorted3(&b));
        /*body_invariant*/drop(forall(|i: usize, j: usize| (0 <= i && i < j && j < res_pos) ==> res[i] <= res[j]));

        // all already sorted are smaller than remaining in a, b
        /*body_invariant*/drop(forall(|i: usize, j: usize| (0 <= i && i < res_pos && a_pos <= j && j < 3) ==> res[i] <= a[j]));
        /*body_invariant*/drop(forall(|i: usize, j: usize| (0 <= i && i < res_pos && b_pos <= j && j < 3) ==> res[i] <= b[j]));

        /*body_invariant*/drop(res_pos > 0 && a_pos < 3 ==> res[res_pos - 1] <= a[a_pos]);
        /*body_invariant*/drop(res_pos > 0 && b_pos < 3 ==> res[res_pos - 1] <= b[b_pos]);
