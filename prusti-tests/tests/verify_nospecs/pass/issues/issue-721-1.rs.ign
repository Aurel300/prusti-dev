...
impl Path {
    ...
    pub fn reset(&mut self) {
        ...
        self.depth = 0;
        self.entries[0] = 0;
        /*assert*/drop(self.entries.len() == self.max_counts.len());
        /*assert*/drop(self.current < MAX_DEPTH);
        /*assert*/drop(self.depth < MAX_DEPTH);
        /*assert*/drop(self.current <= self.depth);
        /*assert*/drop(self.entries[0] == 0);
    }
}
