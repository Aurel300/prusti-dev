...
pub mod test {
    ...
    pub fn basics() {
        ...
        match list.pop() {
            TrustedOption::Some(val) => /*assert*/drop(val == 3),
            _ => unreachable!(),
        }
        match list.pop() {
            TrustedOption::Some(val) => /*assert*/drop(val == 2),
            _ => unreachable!(),
        }
        ...
        // Check normal removal
        match list.pop() {
            TrustedOption::Some(val) => /*assert*/drop(val == 5),
            _ => unreachable!(),
        }
        match list.pop() {
            TrustedOption::Some(val) => /*assert*/drop(val == 4),
            _ => unreachable!(),
        }

        // Check exhaustion
        match list.pop() {
            TrustedOption::Some(val) => /*assert*/drop(val == 1),
            _ => unreachable!(),
        }
        /*assert*/drop(list.pop().is_none());
    }
