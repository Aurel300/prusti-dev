...
fn iterative_fibonacci() {
    ...
    while add_succeeded {
        /*body_invariant*/drop(_ghost_counter >= 1);
        /*body_invariant*/drop(fib(_ghost_counter) == curr);
        /*body_invariant*/drop(fib(_ghost_counter-1) == prev);
...
fn main() {
    ...
    while continue_iteration {
        /*body_invariant*/drop(iter.valid());
        let item = iter.next();
        match item {
            UsizeOption::Some(n) => {
                let i = iter.counter();
