...
pub fn knapsack01_dyn(items: &Items, max_weight: usize) -> ItemIndices {
    ...
    while continue_loop_1 {
        /*body_invariant*/drop(items.len() + 1 == best_value.item_len());
        /*body_invariant*/drop(max_weight + 1 == best_value.weight_len());
        /*body_invariant*/drop(i < items.len());
        /*body_invariant*/drop(0 <= i && i < items.len());
        /*body_invariant*/drop(2 <= max_weight && max_weight < std::usize::MAX);
        /*body_invariant*/drop(forall(|ii: usize, wi: usize|
            (0 <= ii && ii < best_value.item_len() && 0 <= wi && wi < best_value.weight_len()) ==>
            best_value.lookup(ii, wi) >= 0));
        /*body_invariant*/drop(forall(|ii: usize|
        ...
        let mut w = 1;
        while w <= max_weight {
            /*body_invariant*/drop(w <= max_weight);
            /*body_invariant*/drop(items.len() + 1 == best_value.item_len());
            /*body_invariant*/drop(max_weight + 1 == best_value.weight_len());
            /*body_invariant*/drop(0 <= w && w <= best_value.weight_len());
            /*body_invariant*/drop(0 <= i && i < items.len());
            /*body_invariant*/drop(2 <= max_weight && max_weight < std::usize::MAX);
            /*body_invariant*/drop(it.value == items.lookup_value(i));
            /*body_invariant*/drop(it.weight == items.lookup_weight(i));
            /*body_invariant*/drop(forall(|ii: usize, wi: usize|
                (0 <= ii && ii < best_value.item_len() && 0 <= wi && wi < best_value.weight_len()) ==>
                best_value.lookup(ii, wi) >= 0));
            /*body_invariant*/drop(forall(|ii: usize, wi: usize|
                (0 <= ii && ii <= i && 0 <= wi && wi < best_value.weight_len()) ==>
                m(items, ii, wi, max_weight) == best_value.lookup(ii, wi)));
            /*body_invariant*/drop(forall(|wi: usize| (0 <= wi && wi < w) ==>
    ...
    while 0 < i {
        ...
        /*body_invariant*/drop(items.len() + 1 == best_value.item_len());
        /*body_invariant*/drop(max_weight + 1 == best_value.weight_len());
        /*body_invariant*/drop(0 <= left_weight && left_weight <= max_weight);
        /*body_invariant*/drop(forall(|ii: usize, wi: usize|
            (0 <= ii && ii < best_value.item_len() && 0 <= wi && wi < best_value.weight_len()) ==>
            m(items, ii, wi, max_weight) == best_value.lookup(ii, wi)));
        i -= 1;
