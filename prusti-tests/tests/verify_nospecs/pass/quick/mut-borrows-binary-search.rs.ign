...
fn borrow_test2(arr: &mut VecWrapperI32, elem: &mut i32) -> UsizeOption
{
    ...
    while continue_loop {
        /*body_invariant*/drop(0 <= base);
        /*body_invariant*/drop(0 <= size);
        /*body_invariant*/drop(base + size <= arr.len());
        /*body_invariant*/drop(continue_loop ==> size > 2);
        size /= 2;
        let mid = base + size;
        let mid_element = arr.borrow(mid);
...
fn borrow_test4(arr: &mut VecWrapperI32, elem: &mut i32) {
    ...
    while continue_loop {
        /*body_invariant*/drop(0 <= size);
        /*body_invariant*/drop(size <= arr.len());
        /*body_invariant*/drop(continue_loop ==> size > 2);
        size /= 2;
