...
fn capitalize(vec: &mut VecWrapperI32) {
    ...
    while not_finished {
        /*body_invariant*/drop(vec.len() == old(vec.len()));
        /*body_invariant*/drop(0 <= i && i < vec.len());
        /*body_invariant*/drop(forall(|j: usize| (0 <= j && j < i) ==> vec.lookup(j) <= 0));
        /*body_invariant*/drop(forall(|j: usize| (i <= j && j < vec.len()) ==>
                    old(vec.lookup(j)) == vec.lookup(j)));
        /*body_invariant*/drop(forall(|j: usize| (0 <= j && j < i && old(vec.lookup(j)) > 0) ==>
                    -old(vec.lookup(j)) == vec.lookup(j)));
        /*body_invariant*/drop(forall(|j: usize| (0 <= j && j < i && old(vec.lookup(j)) <= 0) ==>
                    old(vec.lookup(j)) == vec.lookup(j)));
...
fn bar() {
    ...
    /*assert*/drop(data.lookup(0) == -1);
    /*assert*/drop(data.lookup(1) == -2);
    /*assert*/drop(data.lookup(2) == -3);
    ...
    /*assert*/drop(data.lookup(1) == -2);
    /*assert*/drop(data.lookup(2) == -3);
    /*assert*/drop(data.lookup(3) == 4);
    /*assert*/drop(data.lookup(4) == -5);
    /*assert*/drop(data.lookup(5) == 6);
}
