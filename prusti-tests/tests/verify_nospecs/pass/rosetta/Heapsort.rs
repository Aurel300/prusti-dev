...
fn heap_sort(array: &mut VecWrapperI32)
{
    ...
    // Create heap
    while continue_loop {
        /*body_invariant*/drop(len == array.len());
        /*body_invariant*/drop(start <= len/2);
        /*body_invariant*/drop(start > 0);
        start -= 1;
    ...
    while continue_loop {
        /*body_invariant*/drop(len == array.len());
        /*body_invariant*/drop(end <= len);
        /*body_invariant*/drop(end > 1);
        end -= 1;
        let start = 0;
...
fn shift_down(array: &mut VecWrapperI32, start: usize, end: usize)
{
    ...
    while continue_loop {
        /*body_invariant*/drop(0 <= root && root < array.len());
        /*body_invariant*/drop(0 <= end && end < array.len());
        /*body_invariant*/drop(array.len() == old(array.len()));
