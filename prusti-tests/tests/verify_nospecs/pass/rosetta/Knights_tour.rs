...
impl Board {
    ...
    fn count_degree(&mut self, p: Point) -> i32 {
        ...
        while continue_loop {
            /*body_invariant*/drop(0 <= i);
            /*body_invariant*/drop(i < moves.len());
...
fn knights_tour(x: i32, y: i32) -> Option<Board> {
    ...
    while continue_loop_1 {
        /*body_invariant*/drop(0 <= p.x && p.x < size());
        /*body_invariant*/drop(0 <= p.y && p.y < size());
        ...
        while continue_loop_2 {
            /*body_invariant*/drop(0 <= i);
            /*body_invariant*/drop(i < candidates.len());
            /*body_invariant*/drop(valid(&min));
