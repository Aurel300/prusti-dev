        ...
        Ok(match assertion.kind {
            ...
            box typed::AssertionKind::SpecEntailment {
                ...
                let qargs_pre = vars.args
                    .iter()
                    .map(|(arg, arg_ty)| self.encode_quantifier_arg(*arg, arg_ty, &forall_pre_id))
                    .collect::<Vec<_>>();
                // TODO: mutable arguments should be duplicated (pre/post state)
                let qargs_post = vars.args
                    .iter()
                    .map(|(arg, arg_ty)| self.encode_quantifier_arg(*arg, arg_ty, &forall_post_id))
                    .collect::<Vec<_>>();
                let qret_post = self.encode_quantifier_arg(vars.result.0, vars.result.1, &forall_post_id);
