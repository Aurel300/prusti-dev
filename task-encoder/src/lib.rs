...
pub type Cache<'tcx, 'vir, E> =
    LinkedHashMap<<E as TaskEncoder>::TaskKey<'tcx>, TaskEncoderCacheState<'vir, E>>;
...
pub trait TaskEncoder {
    ...
    type TaskKey<'tcx>: std::hash::Hash + Eq + Clone + std::fmt::Debug =
        Self::TaskDescription<'tcx>;
    ...
    where
        Self: 'vir,
        F: FnOnce(&'vir CacheRef<'tcx, 'vir, Self>) -> R;
    ...
    fn encode_ref<'tcx: 'vir, 'vir>(
        task: Self::TaskDescription<'tcx>,
