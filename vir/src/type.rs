...
// `Int` and `Perm` are subtypes of `CSnap` since the `prusti_contracts`
// builtins `Int` and `Real` translate to these. The meaning of `Snap` is the
// mathematical representation of any Rust type (which these two types are).
// Note that the `Prim` type is unrelated and other `Prim` types are not `Snap`.
...
impl_exp_type!(CSnap => Snap | Dyn, ref kind if matches!(kind, TypeKind::Int | TypeKind::Perm) || matches!(kind, TypeKind::Domain(name, ..) if name.starts_with("s_") && *name != "s_Param"), "A concrete Prusti snapshot type");
...
impl_exp_type!(Snap => Dyn, ref kind if matches!(kind, TypeKind::Int | TypeKind::Perm) || matches!(kind, TypeKind::Domain(name, ..) if name.starts_with("s_")), "A Prusti snapshot type, either concrete or generic");
